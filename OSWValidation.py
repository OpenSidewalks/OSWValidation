...
from OSWValidation.validator import validate_with_schema
...
if __name__ == '__main__':
    ...
    parser.add_argument("--schemaPath", help="Relative path to read the OSW schema document.")
    ...
    for ind, ways_file in enumerate(ways_files):
        print('Processing the following file : \n{}'.format(ntpath.basename(ways_file)))

        utild = UtilData(ways_file, cf)
        ...
        if cf.do_all_validations or cf.do_schema_validations:
            ...
            with open(ways_file, 'r') as fp:
                ways_geojson = json.load(fp)
            invalid_schema_ways_dict = validate_with_schema(ways_geojson, cf.schemaPath)
